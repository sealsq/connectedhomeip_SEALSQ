...
#include <VaultIC-Elib_292/common/vaultic_common.h>
#include <VaultIC-Elib_292/device/vaultic_2XX_family/vaultic_api.h>

#include "DeviceAttestationVic292Creds.h"
// #include "Vic292CommissionableDataProvider.h"
// #include "Vic292CommisionableInit.h"
...
// Vic292CommisionableDataProvider gCommissionableDataProvider;
...
CHIP_ERROR InitLinuxDeviceOptions()
{
    CERT_DATA verifierVic292 = (CERT_DATA)malloc(VIC292_VERIFIER_SIZE);
    CERT_DATA saltVic292 = (CERT_DATA)malloc(VIC292_SALT_SIZE);
    uint32_t iterationVic292 = 0;

    // Call API functions
    vlt_matter_get_iteration(&iterationVic292);             // Read Iteration stored in VaultIC
    vlt_matter_get_salt(saltVic292);                        // Read Salt stored in VaultIC
    vlt_matter_read_verifier(verifierVic292);               // Read Verifier stroed in VaultIC

    if(iterationVic292 == 0 || saltVic292 == nullptr || saltVic292 == nullptr)
    {
        return CHIP_ERROR_INTERNAL;
    }

    // Cast datas for LinuxDeviceOptions struct
    std::vector<uint8_t> v = std::vector<uint8_t>(verifierVic292, verifierVic292 + VIC292_VERIFIER_SIZE);
    std::vector<uint8_t> s = std::vector<uint8_t>(saltVic292, saltVic292 + VIC292_SALT_SIZE);

    // Add datas in LinuxDeviceOptions struct
    LinuxDeviceOptions::GetInstance().spake2pVerifier.SetValue(v);
    LinuxDeviceOptions::GetInstance().spake2pSalt.SetValue(s);
    LinuxDeviceOptions::GetInstance().spake2pIterations = iterationVic292;
                     ...
                     const Optional<EndpointId> secondaryNetworkCommissioningEndpoint)
{
    ...
    // Set Vic292 Iteration, Salt and Verifier stored in the chip
    err = InitLinuxDeviceOptions();
    SuccessOrExit(err);
...
void ChipLinuxAppMainLoop(AppMainLoopImplementation * impl)
{
...
#if ENABLE_VIC292_DEVICE_ATTESTATION
    SetDeviceAttestationCredentialsProvider(chip::Credentials::Examples::GetCredsVic292());
#else
    ...
    if(vlt_matter_is_init() == TRUE)
    {
        VerifyOrDie(vlt_matter_close()  == CHIP_NO_ERROR);
    }
